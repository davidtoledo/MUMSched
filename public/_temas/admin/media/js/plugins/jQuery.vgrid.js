...
(function($)
{
	...
	function getSize(child)
	{
		...
		_w > 250 && $(child).find('figcaption span').show();
	...
	function animateTo(childs, easing, time, delay)
	{
		...
		for(i=0; i<imax; i++)
		{
			_c = $(childs[i]);
			_pos = _c.position();
			//console.log(_pos.left + " ", _c.data("_vgleft")+ " ", _c.offset().left+ " ", _c.parent().offset().left,  "       ",Number(_c.offset().left) - Number( _c.parent().offset().left ))
			if(_pos.left + 100 != _c.data("_vgleft") || _pos.top != _c.data("_vgtop")) // Alteração para funcionar no IE9 abaixo
			{
				isMove = true;
			}
		}
